  ...
  useEffect(() => {
    const fetchAndMergeData = async () => {
      try {
        ...
        const reviewsResponse = await fetch("http://localhost:5000/doctor/reviews", {
          method: "GET",
          headers: {
            'authorization': `Bearer ${token}`,
            'User-Id': id,
        ...
        if (!patientsResponse.ok || !reviewsResponse.ok) {
          throw new Error(`HTTP error! status: ${patientsResponse.status} ${reviewsResponse.status}`);
        }
        ...
        const merged = reviews.map((review) => {
          ...
          if (patient) {
            ...
            const formattedDate = new Date(review.date_issue).toISOString().slice(0, 10);
