...
receptionist_routes.put('/edit-appointment/:id', async (req, res) => {
    ...
    try {
        ...
        const appointmentStartDateTime = new Date(`${date}T${start_time}:00`);
        const appointmentEndDateTime = new Date(`${date}T${end_time}:00`);
        ...
        const conflictingAppointments = await pool.query(
            ...
            [doctor_id, date, appointment_id, end_time, start_time]
        );
        ...
        const updatedAppointment = await pool.query(
            "UPDATE appointment SET doctor_id = $1, date = $2, start_time = $3, end_time = $4, status = $5 WHERE appointment_id = $6 AND patient_id = $7 RETURNING *;",
            [doctor_id, date, start_time, end_time, status, appointment_id, patient_id]
        );
