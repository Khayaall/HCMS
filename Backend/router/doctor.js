...
doctor_routes.post(
  ...
  async (req, res) => {
    ...
    try {
      await pool.query(
        "UPDATE doctor SET image_url = $1 WHERE doctor_id = $2",
        [imageUrl, d_id]
      );
...
doctor_routes.get("/view_image", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT image_url FROM doctor WHERE doctor_id = $1",
      [d_id]
    );
...
doctor_routes.get("/patients", async (req, res) => {
  ...
  const patients_id = await pool.query(
    "SELECT * FROM appointment WHERE doctor_id = $1",
    [actual_doctor_id]
  );
  const patients = await pool.query(
    "SELECT * FROM patient WHERE patient_id = ANY($1)",
    [patients_id.rows.map((row) => row.patient_id)]
  );
...
doctor_routes.get("/reviews", async (req, res) => {
  try {
    const rating_reviews = await pool.query(
      "SELECT * FROM rating_review WHERE doctor_id = $1",
...
doctor_routes.post("/edit_profile", async (req, res) => {
  ...
  values.push(d_id);
  const query = `UPDATE doctor SET ${updates.join(", ")} WHERE doctor_id = $${
    values.length
  }`;
  ...
  try {
    ...
    const table_name =
      patient_type === "obstetrics"
        ? "obstetrics_medical_record"
        : "infant_medical_record";
    await pool.query(
    ...
    if (req.body.diagnosis) {
      await pool.query(
        "UPDATE appointment SET diagnosis = $1 WHERE appointment_id = $2",
        [req.body.diagnosis, appointment_id]
      );
      await pool.query(
    ...
    if (req.body.treatment) {
      await pool.query(
        "UPDATE appointment SET treatment = $1 WHERE appointment_id = $2",
        [req.body.treatment, appointment_id]
      );
      await pool.query(
    ...
    if (req.body.medication || req.body.note) {
      const date_issue_result = await pool.query(
        "SELECT date FROM appointment WHERE appointment_id = $1",
        [appointment_id]
      );
    ...
    if (req.body.medication) {
      await pool.query(
        "UPDATE prescription SET medication = $1 WHERE appointment_id = $2",
        [req.body.medication, appointment_id]
      );
    }

    if (req.body.note) {
      await pool.query(
        "UPDATE prescription SET note = $1 WHERE appointment_id = $2",
        [req.body.note, appointment_id]
      );
    }
...
doctor_routes.get("/get_todays_appointments", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("An error occurred while fetching today's appointments");
  }
});
...
doctor_routes.get("/get_upcoming_appointments", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("An error occurred while fetching upcoming appointments");
  }
});
...
doctor_routes.get("/statistics", async (req, res) => {
  ...
  try {
    ...
    const totalPatients = await pool.query(
      "SELECT COUNT(DISTINCT patient_id) FROM appointment WHERE doctor_id = $1",
      [d_id]
    );
    const totalReviews = await pool.query(
      "SELECT COUNT(*) FROM rating_review WHERE doctor_id = $1",
      [d_id]
    );
    const totalPrescriptions = await pool.query(
      "SELECT COUNT(*) FROM prescription WHERE doctor_id = $1",
      [d_id]
    );
...
doctor_routes.post(
  "/edit_medical_record/:patient_id",
  upload.single("image"),
  async (req, res) => {
    const d_id = req.session.authorization.id;
    ...
    try {
      ...
      const patient_type =
        patient_type_result.rows[0].patient_type.toLowerCase();
      ...
      try {
        ...
        res
          .status(500)
          .send("An error occurred while updating the medical record");
      }
    } catch (error) {
      console.error("Error retrieving patient type:", error);
      res
        .status(500)
        .send("An error occurred while retrieving the patient information");
    }
  }
);
...
doctor_routes.post("/treatment_plan/:patient_id", async (req, res) => {
  const p_id = req.params.patient_id;
  const d_id = req.session.authorization.id;
  patient_type_result = await pool.query(
    "SELECT patient_type FROM patient WHERE patient_id = $1",
    [p_id]
  );
  let fields = [
...
doctor_routes.put("/edit_treatment_plan/:record_id", async (req, res) => {
  ...
  const specialty_result = await pool.query(
    "SELECT specialty FROM doctor WHERE doctor_id = $1",
    [doctor_id]
  );
  const specialty = specialty_result.rows[0].specialty;
  console.log("specialty:", specialty);
  let p_id = 0;
  if (specialty === "obstetrics") {
    p_id_result = await pool.query(
      "SELECT patient_id FROM cancer_treatment_plan WHERE id = $1",
      [record_id]
    );
  ...
  if (specialty === "infant") {
    ...
    p_id_result = await pool.query(
      "SELECT patient_id FROM infant WHERE id = $1",
      [record_id]
    );
    if (p_id_result.rows.length === 0) {
      console.log(p_id_result.rows);
...
doctor_routes.get(
  "/get_cancer_treatment_plan/:patient_id",
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .send("An error occurred while fetching the treatment plan");
    }
  }
);
