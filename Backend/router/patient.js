...
patient_routes.get('/appointments', async (req, res) => {
    ...
    return res.status(200).send(appointments.rows);
        ...
        return res.status(500).send('An error occurred while fetching appointments of patient.');
    }
});
...
patient_routes.get('/browse-doctors', async (req, res) => {
    ...
    return res.status(200).send(type_doctors.rows);
        ...
        return res.status(500).send('An error occurred while fetching same-type doctors of patient.');
    }
});
...
patient_routes.get('/browse-selected-doctors', async (req, res) => {
    ...
    return res.status(200).send(type_doctors.rows)
        ...
        return res.status(500).send('An error occurred while fetching selected doctors.');
    }
});
...
patient_routes.get('/prescriptions', async (req, res) => {
    ...
    return res.status(200).send(prescriptions.rows);
        ...
        return res.status(500).send('An error occurred while fetching prescriptions of patient.');
    }
});
...
patient_routes.get('/ultra-images', async (req, res) => {
    ...
    return res.status(200).send(ultra_images.rows);
        ...
        return res.status(500).send("An error occured while fetching patient\'s ultra images")
    }
});
...
patient_routes.get('/medical-record', async (req, res) => {
    ...
    try{
        const result = await pool.query(format("SELECT * FROM %s_medical_record WHERE patient_id = %s",type.rows[0].patient_type,p_id));
        if(result.rows.length === 0){
        ...
        return res.status(500).send('An error occurred while fetching patient\'s medical record');
    }
});
...
patient_routes.get('/statistics',async (req, res) => {
    const p_id = req.session.authorization.id;
    try{
        const total_doctors = await pool.query("SELECT COUNT(*) FROM doctor WHERE doctor_id = (SELECT DISTINCT doctor_id FROM appointment WHERE patient_id = $1);",[p_id]);
...
patient_routes.post('/new-appointment', async (req, res) => {
    ...
    try {
        ...
        var appointment_id = await pool.query("SELECT MAX(appointment_id) FROM appointment;");
        ...
        const newAppointment = await pool.query(
            ...
            [appointment_id, patient_id, doctor_id, date, start_time, appointmentEndDateTime.toTimeString().slice(0, 8), 'Scheduled']
        );
...
patient_routes.post('/upload-image', upload.single('image'), async (req, res) => {
    const actual_patient_id = req.session.authorization.id;
